...
| **CLAUDE.md** | Personalized AI agent context file, auto-generated from the three previous documents: development commands, fixed stack with ADR references, directory structure, design tokens, domain invariants, build sequence, environment variables, edge cases, glossary, and execution rules. Under 300 words, ready to drop into the project repository |
...
Claude generates a personalized `CLAUDE.md` file automatically — no additional questions needed. Every value is extracted from the three previous documents, never invented.

The generated `CLAUDE.md` covers:

- **Project overview** — name, one-line description, stack, paths to the three docs
- **Development commands** — literal commands for setup, dev, build, lint, format, tests, and migrations, ready to run
- **Mandatory code patterns** — language conventions, file naming, ID and timestamp format, error base class, DB and API access patterns
- **What to NEVER do** — explicit prohibitions, each traced back to the ADR or invariant that originates it, plus a standing instruction to record corrections whenever the agent makes a mistake
- **Fixed stack and versions** — with ADR references; the agent treats these as closed decisions and does not propose alternatives
- **Directory structure** — first-level folders with responsibility, so the agent knows where to create each file
- **Design tokens** — brand and semantic colors, fonts, spacing scale (omitted for CLI, API, or library products)
- **Critical domain invariants** — every INV-XX in compact form with its type and where to verify it in code
- **Build sequence** — steps with checkpoints, plus a current-step field for the developer to track progress
- **Environment variables** — name, default, and whether required
- **Answered edge cases** — situations the agent would otherwise implement wrong
- **Domain glossary** — the term to use in code versus the ones to avoid
- **Execution rules** — a fixed section in every generated file: touch only what the request requires, never refactor adjacent code, remove only the orphans your own changes created, declare ambiguity instead of guessing, present multiple interpretations rather than picking silently

The file is kept under 300 words. Since Claude Code loads it at the start of every session, every token it costs is spent before you even type your prompt — so it carries only what changes the agent's behavior.

Drop `CLAUDE.md` into the root of your project repository.
...
└── references/
    ...
    └── claude-md.md                # CLAUDE.md generation rules, section sources, and word limit
```
